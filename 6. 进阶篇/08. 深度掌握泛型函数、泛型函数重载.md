...
## 【泛型函数】—泛型函数+快速排序算法【比冒泡稍复杂】 【透彻理解泛型函数的好处】

**泛型函数安排：**

1. 大致讲解快速排序算法
2. 编写字符串排序、整数排序函数
3. 泛型函数实现字符串排序、整数排序【泛型函数带来的好处】
...
**(3)  泛型函数实现字符串排序，整数排序【泛型函数带来的好处】** **泛型类的格式**：
...
## 【泛型函数重载】泛型函数重载准备【经典复杂排序器】—自排序、多种数据类型、中文排序
...
## 【泛型函数重载】泛型函数重载重构自排序丶多种数据类型丶中文排序 【彻底理解泛型函数重载带来的巨大优势】
...
泛型工厂函数应用场景：
- 使用场景1：在一些不方便或者没有办法直接 new  类名（）格式来创建对象，例如：后面的装饰器中就多次用到。
...
## 【泛型函数+交叉类型+类型断言+枚举】 深度理解泛型函数+交叉类型综合应用
...
## 彩蛋：【泛型函数重载+交叉类型+类型断言】 再度加深掌握交叉类型的综合应用【更上一层楼】
